...
from model.fastspeech2 import (FastSpeech2, load_fs2_state, masked_mse, masked_l1,
...
def validate(model, val_loader, device):
    """Returns per-element masked L1 mel loss over the validation set,
    teacher-forced like training."""
    model.eval()
    losses = []
    with torch.no_grad():
        for vbatch in val_loader:
            ...
            with autocast("cuda", enabled=tcfg.fp16):
                mel_pred, _, _, _, _ = model(
                    vph, vphl, durations_gt=vdur, f0_gt=vf0, energy_gt=veng)
                T = min(mel_pred.size(1), vmel.size(1))
                lens = vmell.clamp(max=T)
                losses.append(masked_l1(mel_pred[:, :T], vmel[:, :T], lens).item() / acfg.n_mels)
...
def train(resume_path=None, max_steps=None):
    ...
    if ckpt is not None:
        ...
        # or a param count change like dropping the old joint PostNet), a
        # naive load silently assigns momentum buffers to the wrong
    ...
    for step in pbar:
        ...
        with autocast("cuda",enabled=tcfg.fp16):
            ...
            # MSE is the objective the decoder converges under; L1 is less
            # prone to conditional-mean blur, giving sharper harmonic detail.
            # Use silence-weighted L1 to prevent silence frames from being diluted in loss
            loss_mel_mse = masked_mse(mel_pred[:, :T], mel_gt[:, :T], capped_lens)
            loss_mel_l1  = masked_l1_silence_weighted(
                mel_pred[:, :T],  mel_gt[:, :T], capped_lens,
                energy_gt=energy_gt[:, :T], silence_weight=2.0
            )
            loss_mel    = (loss_mel_mse + loss_mel_l1) * tcfg.mel_loss_weight
        ...
        if (step + 1) % tcfg.log_every == 0:
            ...
            writer.add_scalar("train/mel_mse", loss_mel_mse.item(), step + 1)
            writer.add_scalar("train/mel_l1",  loss_mel_l1.item(),  step + 1)
            # per-element view — masked losses sum over the 80 mel channels
            writer.add_scalar("train/mel_l1_per_elem",
                              loss_mel_l1.item() / acfg.n_mels, step + 1)
        ...
        if (step + 1) % tcfg.val_every == 0:
            ...
            print(f"\n[step {step+1}] val mel L1/elem: {val_loss:.5f}")
            writer.add_scalar("val/mel_l1", val_loss, step + 1)
...
if __name__ == "__main__":
    ...
    train(resume_path=resume_path, max_steps=args.max_steps)