"""
HiFi-GAN Generator — matches the official jik876/hifi-gan architecture exactly
(flat `resblocks` list, not grouped per upsample-stage), so official pretrained
checkpoints (LJ_V1, UNIVERSAL_V1) load directly via `load_state_dict` with no
key remapping. Used for both from-scratch training and fine-tuning from those
checkpoints — see train_hifigan.py's --init-g/--init-d flags.
